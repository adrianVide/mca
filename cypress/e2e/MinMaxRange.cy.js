describe('MinMaxRange Component', () => {
    ...
    it('displays range slider after fetching data', () => {
      cy.intercept('GET', 'http://demo1209414.mockable.io/minmax', { fixture: 'min_max_values.json' }).as('fetchRangeValues');
  
      cy.wait('@fetchRangeValues');
    ...
    it('allows dragging and updating min value', () => {
      ...
      cy.get('.values .value-label').eq(0).should('contain', '99€');
    });
    ...
    it('allows dragging and updating max value', () => {
      ...
      cy.get('.values .value-label').eq(1).should('contain', '83.66€');
    });
  });
